...
| 🚀 **상태바 메뉴** | 우측 하단 🚀 아이콘으로 Start/Stop/Restart/Logs/Settings 빠른 접근 |
| 📊 **실행 상태 표시** | Tomcat 실행 중이면 상태바에 `✔` 표시 |
| 🔄 **Restart 커맨드** | Stop → Start를 한 번에 처리 |
| 🌍 **크로스 플랫폼** | Windows(`.bat`) / Mac·Linux(`.sh`) 스크립트 모두 생성 |
| 🔨 **빌드 연동** | Tomcat 시작 전 Maven/Gradle 빌드 자동 실행 옵션 |
| 🌐 **브라우저 자동 오픈** | 서버 기동 완료 시 브라우저 자동 실행 |
| 🧬 **JNDI & Context 병합** | `META-INF/context.xml` 내용 자동 삽입 |
