...
- **JNDI & Context 자동 병합**: 프로젝트의 `META-INF/context.xml` 내용을 자동으로 읽어와 Tomcat 설정에 병합해줍니다. `settings.json`의 JNDI 추가 설정 가능
...
- **`docBase` 설정 필수**: Tomcat이 실행될 웹 애플리케이션의 루트 경로(`docBase`)가 정확해야 합니다. 기본적으로 `${workspaceFolder}/target/exploded` 등을 바라보며, 실제 빌드된 폴더 경로로 수정이 필요할 수 있습니다.
...
- `happySpringTomcat.docBase`: Tomcat이 서빙할 웹앱 루트 디렉토리 (빌드된 war 또는 exploded 폴더).
