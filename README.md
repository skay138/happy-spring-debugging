...
- **⚡ 한 번의 클릭으로 설정**: `Happy Spring Tomcat: Apply Debug Setup` 명령어로 디버깅에 필요한 모든 스크립트와 설정 파일을 생성합니다.
...
3. `Ctrl+Shift+P` (또는 `F1`)를 눌러 커맨드 팔레트를 열고 **`Happy Spring Tomcat: Apply Debug Setup`**를 실행합니다.
...
- **`docBase` 자동 감지**: `Apply Debug Setup` 실행 시 빌드 결과물 경로를 **자동으로 감지**합니다. 만약 감지되지 않거나 여러 개일 경우, 상단 퀵픽(QuickPick) 창에서 올바른 폴더를 선택하기만 하면 됩니다.
