...
export function activate(context: vscode.ExtensionContext) {
    let disposable = vscode.commands.registerCommand('happy-spring-debugging.setup', async () => {
        ...
        const config = vscode.workspace.getConfiguration('happySpringDebugging');
        ...
        if (!tomcatHome) {
            const selectedHome = await vscode.commands.executeCommand<string>('happy-spring-debugging.selectTomcatHome');
            if (selectedHome) {
        ...
        if (!fs.existsSync(resolvedDocBase)) {
            ...
            if (pick === 'Yes') {
                const selectedDocBase = await vscode.commands.executeCommand<string>('happy-spring-debugging.selectDocBase');
                if (selectedDocBase) {
                    ...
                    if (refreshedDocBase !== resolvedDocBase) {
                        ...
                        vscode.commands.executeCommand('happy-spring-debugging.setup');
                        return;
                    }
                }
            }
        ...
        // --- Setup .vscode/happy-spring-debugging/ ---
        const extensionDir = path.join(vscodeDir, 'happy-spring-debugging');
...
<!--
  Auto-generated by happy-spring-debugging extension.
...
`;
        ...
        const stopTaskDef = {
            "label": "Stop Tomcat",
            "type": "shell",
            "command": "\${workspaceFolder}\\\\.vscode\\\\happy-spring-debugging\\\\stop-tomcat.bat",
            "presentation": {
                "reveal": "silent",
                "panel": "shared",
                "close": true,
        ...
        const startTaskDef = {
            ...
            "type": "shell",
            "command": "\${workspaceFolder}\\\\.vscode\\\\happy-spring-debugging\\\\start-tomcat.bat",
            "isBackground": true,
            "problemMatcher": {
                "pattern": { "regexp": "^$" },
                "background": {
    ...
    let selectTomcatHomeDisposable = vscode.commands.registerCommand('happy-spring-debugging.selectTomcatHome', async () => {
    ...
    let selectDocBaseDisposable = vscode.commands.registerCommand('happy-spring-debugging.selectDocBase', async () => {
        const workspaceFolders = vscode.workspace.workspaceFolders;
    ...
    let configListener = vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration('happySpringDebugging')) {
